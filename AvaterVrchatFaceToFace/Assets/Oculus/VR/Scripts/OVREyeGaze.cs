...
public class OVREyeGaze : MonoBehaviour
{
    ...
    private OVRPlugin.EyeGazesState _currentEyeGazesState, eyeGazesStateCpy;
    ...
    public OVRControllerServer ovrEyeController;

    private string filePathEyes => ovrEyeController.filePathEyes;
    ...
    private void Update()
    {
        ...
        if (ovrEyeController.allStringGet.GetAllString() != null)
        {
            Vector3 vector = new(0, 0, 0);
            Quaternion quaternion = new(0, 0, 0, 0);

            ovrEyeController.TrySetEyeGaze(ApplyPosition, ApplyRotation, ref vector, ref quaternion, Eye);
            if (ApplyPosition)
            {
            ...
            if (ApplyRotation)
            {
                transform.rotation = CalculateEyeRotation(quaternion);
            }
        }
    }
    ...
    /// <returns></returns>
    public bool TryGetEyeGazeState(EyeId eye,out Vector3 position,out Quaternion rotation)
    {
        if (eye < 0 || eye > EyeId.Max)
        {
        ...
        else
        {
            ...
            OVRPlugin.GetEyeGazesState(OVRPlugin.Step.Render, -1, ref eyeGazesStateCpy);
            var pose = eyeGazesStateCpy.EyeGazes[(int)Eye].Pose.ToOVRPose();
            //重新获取
            switch (TrackingMode)
            {
                case EyeTrackingMode.HeadSpace:
    ...
    public Quaternion CalculateEyeRotation(Quaternion eyeRotation)
    {
    ...
    private OVREyeGaze.EyeId[] eyeIds = new OVREyeGaze.EyeId[]{
