...
public class OVRFaceExpressions : MonoBehaviour/*, IReadOnlyCollection<float>*/
{
    ...
    public OVRControllerServer ovrFaceController;
    
    //private int indexOfFrame;
    private string[] stringArray;
    private string filePathFace => ovrFaceController.filePathFace;
    ...
    /// changed
    /// </summary>
    public OVRPlugin.FaceState _currentFaceState, faceStateCpy;
    ...
    private void Update()
    {
        ...
        OVRPlugin.GetFaceState(OVRPlugin.Step.Render, -1, ref faceStateCpy);
        //已经get到传入的数据
        if (ovrFaceController.allStringGet.GetAllString() != null)
        {
            ovrFaceController.TrySetFaceExpression(ref _currentFaceState);
            //eye和body在对应的ovr脚本中实现
        }
    }
    ...
    {
        if (!File.Exists(filePathFace))
        {
            File.Create(filePathFace).Dispose();
        }

        // 使用 StreamWriter 打开文件，并写入文本
        using (StreamWriter writer = new StreamWriter(filePathFace, true))
        {
    ...
    public bool TryGetFaceExpressionWeight(FaceExpression expression, out float weight)
    {
        ...
        weight = faceStateCpy.ExpressionWeights[(int)expression];
        return true;
    }
    ...
    private OVRFaceExpressions.FaceExpression[] interstedFaceExpressions = new OVRFaceExpressions.FaceExpression[] {
